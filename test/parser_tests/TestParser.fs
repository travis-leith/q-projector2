...
let testAtoms = 
  let testAtom name s f = test name {
    Parser.parseStatement s |> Expect.equal "" (s.TrimEnd() |> f |> Ok)
  }
...
let testIdentifier =
  let mixed = test "Mixed" {
    "someId1" |> Parser.parseStatement |> Expect.equal "" ("someId1" |> Identifier.Identifier |> Expression.IdentExpr |> Ok)
  }

  let single = test "Single" {
    "a" |> Parser.parseStatement |> Expect.equal "" ("a" |> Identifier.Identifier |> Expression.IdentExpr |> Ok)
  }
...
let testBinaryExpressions =
  let singleNoSpace = test "SingleNoSpace" {
    ...
    "1*b" |> Parser.parseStatement |> Expect.equal "" (expectation |> Ok)
  }
  ...
  let singleWithSpace = test "SingleWithSpacec" {
    ...
    "b * 1" |> Parser.parseStatement |> Expect.equal "" (expectation |> Ok)
  }
  ...
  let double = test "Double" {
    ...
    "1 * b +3" |> Parser.parseStatement |> Expect.equal "" (expectation |> Ok)
  }
...
let testFunctions = 
  let oneParam = test "OneParam" {
    ...
    Parser.parseStatement "{[x] 1 +   x}" |> Expect.equal "" expected
  }
  ...
  let noParam = test "NoParam" {
    ...
    Parser.parseStatement "{[] 1 + x}" |> Expect.equal "" expected
  }
  ...
  let twoParam = test "TwoParam" {
    ...
    Parser.parseStatement "{[x; y] y + x}" |> Expect.equal "" expected
  }
